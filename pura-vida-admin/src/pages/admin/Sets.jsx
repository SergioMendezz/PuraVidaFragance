...
import { getSets, postSet, putSet, deleteSet, getPerfumes, getBodys, getBodySprays } from "../../services/api";

const emptySet  = { nombre: "", precio: "", descripcion: "", imagenUrl: "" };
...
export default function Sets() {
  ...
  const load = async () => {
    try {
      const [sRes, pRes, bRes, spRes] = await Promise.all([getSets(), getPerfumes(), getBodys(), getBodySprays()]);
  ...
  const openEdit = (s) => {
    setEditing(s);
    setForm({ nombre: s.nombre, precio: s.precio, descripcion: s.descripcion ?? "", imagenUrl: s.imagenUrl ?? "" });
  ...
  // Opciones de productos según tipo
  ...
  const updateItem = (i, key, val) => {
    ...
    // Al cambiar tipo, resetear idProducto
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[#F0F0F0]">
                    {["Nombre", "Precio", "Items", "Descripción", "Acciones"].map(h => (
                ...
                <tbody>
                  ...
                  {sets.length === 0 && (
                    <tr><td colSpan={5} className="px-5 py-8 text-center text-sm text-gray-400">No hay sets registrados</td></tr>
                  )}
                </tbody>
              </table>
            </div>
